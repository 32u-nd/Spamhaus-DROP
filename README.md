...
Because the output file lives in this repository, it is always reachable at a permanent raw URL:
...
1. **Fetch ASNs** — downloads the [Spamhaus ASN-DROP list](https://www.spamhaus.org/drop/asndrop.json) and extracts all listed Autonomous System Numbers. ASNs are used only as an intermediate lookup key and do not appear in the output.
...
3. **Resolve ASNs via RIPEstat** — for each ASN, queries the [RIPEstat announced-prefixes API](https://stat.ripe.net/data/announced-prefixes/data.json) to retrieve all currently routed prefixes. RIPEstat lookups are retried up to **2 times** on transient errors.
...
The workflow file lives at `.github/workflows/update-blocklist.yml`.
...
The bot commit message includes the UTC timestamp so you can trace exactly when each update was generated.
...
`ipaddress` and `json` are part of the Python standard library — no additional dependencies needed.
